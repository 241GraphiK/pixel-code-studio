...
export default function ResourceUpload({ courseId, onUploaded }: ResourceUploadProps) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ACCEPTED_TYPES[f.type]) {
      toast.error("Type de fichier non supporté. Utilisez PDF, PNG, JPG ou WebP.");
      return;
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const resourceType = ACCEPTED_TYPES[file.type] || "link";
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm">
          <Upload className="w-4 h-4 mr-1" /> Ajouter une ressource
        </Button>
      </DialogTrigger>
          ...
          <div>
            ...
            {file ? (
              <div
                ...
                className="flex items-center gap-3 p-4 rounded-lg border border-border bg-muted/30 cursor-pointer hover:bg-muted/50 transition-colors"
              >
                {fileIcon}
              ...
              <div
                onClick={() => inputRef.current?.click()}
                className="flex flex-col items-center gap-2 p-6 rounded-lg border-2 border-dashed border-border cursor-pointer hover:border-primary/50 hover:bg-primary/5 transition-all"
              >
